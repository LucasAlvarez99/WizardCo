...
function CartProvider({ children }) {
  ...
  const applyCoupon = useCallback(
    (rawCode) => {
      ...
      if (appliedCoupon) {
        return { success: false, message: `Ya tenés el cupón ${appliedCoupon.code} aplicado. Quitalo para usar otro.` };
      }

      const found = coupons.find((c) => c.code === code);

      if (!found) {
        return { success: false, message: "El cupón no existe o ya no está disponible." };
      }
      ...
      setCoupons((prev) => {
        ...
        return prev.map((c) => (c.code === code ? { ...c, usableCount: newCount } : c));
      });

      setAppliedCoupon({ code: found.code, discountPercentage: found.discountPercentage });
